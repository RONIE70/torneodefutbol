...
if(isset($_POST['grabar']))
{
	...
	{
		if($_POST['accion'] == "insertar")$sql = new mysqli ("INSERT into jugadores(idequipo,jugador,nro,goles) values ('".$_POST['idequipo']."','".$_POST['jugador']."','".$_POST['nro']."','".$_POST['goles']."')");
		elseif($_POST['accion'] == "modificar")$sql = new mysqli ("UPDATE jugadores SET idequipo = '".$_POST['idequipo']."', jugador = '".$_POST['jugador']."', goles = '".$_POST['goles']."' WHERE idjugador = ".$_POST['idjugador']);		
	}
}
elseif(isset($_GET['eliminar']))$sql = new mysqli ("DELETE from jugadores where idjugador = ".$_GET['id']);
...
{
	$sql = new mysqli ("SELECT idjugador,idequipo,jugador,nro,goles FROM jugadores WHERE idjugador = ".$_GET['id']);
	$resultado = mysqli_query($sql, $enlace);	
...
$sql = new mysqli ("SELECT j.idjugador,e.idequipo,e.equipo,j.jugador,j.nro,j.goles FROM jugadores j, equipos e WHERE e.idequipo = j.idequipo ORDER BY j.goles desc, e.idequipo, j.nro");
$resultado = mysqli_query($sql, $enlace);
...
// Combo Equipo
$sql = new mysqli ("SELECT idequipo,idzona,equipo FROM equipos order by idequipo");
$resultado = mysqli_query($sql, $enlace);
...
if(isset($_POST['publicar']) && isset($_POST['id']) && $_POST['id'] == 3)
{
	...
	$sql = new mysqli ("SELECT j.idjugador,e.idequipo,e.equipo,j.jugador,j.nro,j.goles FROM jugadores j, equipos e WHERE e.idequipo = j.idequipo ORDER BY e.equipo");
	$resultado = mysqli_query($sql, $enlace);
	$count = mysqli_num_rows($resultado);
	if($count > 0)
	{
